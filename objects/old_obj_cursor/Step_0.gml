...
if (old_obj_battle_manager.unit_hovered)
{
	...
	obj_battle_state_machine.current_state == BattleState.PLAYER_TURN_ATTACK)
{
	...
	if (objInputManager.held.right) { _input_x += CURSOR_BASE_SPEED; }
	if (objInputManager.held.left)  { _input_x -= CURSOR_BASE_SPEED; }
	if (objInputManager.held.down)  { _input_y += CURSOR_BASE_SPEED; }
	if (objInputManager.held.up)    { _input_y -= CURSOR_BASE_SPEED; }

	// Detect left analog stick
	if (abs(objInputManager.left_analog_x) > objInputManager.gamepad_deadzone ||
		abs(objInputManager.left_analog_y) > objInputManager.gamepad_deadzone) {
		_input_x += objInputManager.left_analog_x;
		_input_y += objInputManager.left_analog_y;
	}
	...
	{
		if (objInputManager.held.sprint) { cursor_speed = CURSOR_SPRINT_SPEED }
